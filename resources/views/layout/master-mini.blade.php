...
<head>
  ...
  <style>
    ...
    .di-navbar { position: fixed; top: 0; left: 0; right: 0; z-index: 10; background: rgba(16,44,79,0.95); color: #fff; }
    .di-navbar .container { display: flex; align-items: center; justify-content: space-between; padding: 10px 0; }
    .di-brand { display: flex; align-items: center; color: #fff; text-decoration: none; font-weight: 600; }
    .di-brand img { height: 28px; margin-right: 10px; }
    html[dir='rtl'] .di-brand img { margin-right: 0; margin-left: 10px; }
    .di-nav-links { display: flex; align-items: center; }
    .di-nav-links a { color: #fff; margin: 0 12px; text-decoration: none; opacity: .95; }
    .di-nav-links a:hover { opacity: 1; }
    .di-nav-toggle { display: none; background: transparent; border: none; color: #fff; font-size: 24px; }
    @media (max-width: 992px) {
      .di-nav-links { display: none; position: absolute; top: 100%; left: 0; right: 0; background: rgba(16,44,79,0.98); padding: 10px 0; }
      .di-nav-links.show { display: block; }
      .di-nav-links a { display: block; padding: 8px 16px; }
      .di-nav-toggle { display: inline-block; }
    }
    ...
    .di-footer { padding-top: 3rem; padding-bottom: 2.5rem; }
    #contact { background-color: #102c4f; box-shadow: 0 -18px 40px rgba(0,0,0,0.4); }
    #contact h6 { font-weight: 700; letter-spacing: .04em; text-transform: uppercase; }
  ...
  <div class="container-scroller" id="app">
    ...
    <nav class="di-navbar">
      <div class="container">
        ...
        <button class="di-nav-toggle" type="button" aria-label="Toggle navigation"><span class="mdi mdi-menu"></span></button>
        <div class="di-nav-links" id="diNavLinks">
          ...
          @foreach($locales as $code => $props)
            <a href="{{ Mcamara\LaravelLocalization\Facades\LaravelLocalization::getLocalizedURL($code, null, [], true) }}">{{ $props['native'] }}</a>
          @endforeach
        </div>
      </div>
    </nav>
      ...
      <div class="container di-footer">
        <footer class="row">
          <div class="col-md-4 mb-3">
            ...
            <p class="mt-3" style="opacity:.95; color:#fff;">{{ __('We are a management consulting firm helping organizations make data-driven decisions.') }}</p>
            <div class="mt-2">
              <a href="#" class="di-me-2" style="color:#fff; opacity:.9;"><i class="mdi mdi-twitter"></i></a>
              <a href="#" class="di-me-2" style="color:#fff; opacity:.9;"><i class="mdi mdi-linkedin"></i></a>
              <a href="#" class="di-me-2" style="color:#fff; opacity:.9;"><i class="mdi mdi-facebook"></i></a>
            </div>
          </div>
          <div class="col-md-4 mb-3">
            <h6 class="mb-2">{{ __('app.resources') }}</h6>
            <ul class="list-unstyled" style="opacity:.9;">
              <li><a href="{{ route('landing') }}#features" style="color:#fff;">{{ __('Key Benefits') }}</a></li>
              <li><a href="{{ route('landing') }}#plans" style="color:#fff;">{{ __('Choose Your Plan') }}</a></li>
            </ul>
          </div>
          <div class="col-md-4 mb-3">
