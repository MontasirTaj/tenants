      ...
      <div class="col-12 mb-4">
        <div class="card h-100">
          <div class="card-body">
            ...
            @endif
            <form method="POST" action="{{ route($prefix.'.users.store', ['subdomain' => request()->route('subdomain')]) }}">
              @csrf
              <div class="mb-3">
        ...
        <div class="card h-100">
          <div class="card-body">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-2">
              ...
              <div class="mt-3 mt-md-0">
                <div class="btn-group btn-group-sm" role="group" aria-label="{{ __('app.export') }}">
                  <a href="{{ route($prefix.'.users.export.excel', ['subdomain' => $sub]) }}" class="btn btn-outline-success">
                    <i class="mdi mdi-file-excel-outline"></i>
                    <span>{{ __('app.export_excel') }}</span>
                  </a>
                  <a href="{{ route($prefix.'.users.export.pdf', ['subdomain' => $sub]) }}" class="btn btn-outline-danger">
            ...
            <div class="table-responsive tenant-table-wrapper">
              <table class="table table-striped table-hover align-middle tenant-table" id="tenant-users-table">
                <thead>
                  <tr>
                    <th>{{ __('app.name') }}</th>
                    <th>{{ __('app.email') }}</th>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="text-center">
                      <a href="{{ route($prefix.'.users.edit', ['subdomain' => request()->route('subdomain'), 'user' => $u->id]) }}" class="btn btn-sm btn-outline-primary tenant-action-btn">
                        <i class="mdi mdi-pencil-outline"></i>
                        <span>{{ __('app.edit') }}</span>
                      </a>
                      <form method="POST" action="{{ route($prefix.'.users.destroy', ['subdomain' => request()->route('subdomain'), 'user' => $u->id]) }}" class="d-inline-block" onsubmit="return confirm('{{ __('app.confirm_delete_user') }}');">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-sm btn-outline-danger tenant-action-btn" type="submit">
