        ...
        <div class="card h-100">
          <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-2">
              ...
              <a href="{{ route('tenant.subdomain.roles.with-permissions', ['subdomain' => request()->route('subdomain')]) }}" class="btn btn-outline-secondary btn-sm">
                {{ __('app.roles_with_permissions_link') }}
              </a>
            </div>
            <p class="text-muted mb-3">{{ __('app.roles_intro') }}</p>
            <form method="POST" action="{{ route('tenant.subdomain.roles.store', ['subdomain' => request()->route('subdomain')]) }}" class="mb-3">
              @csrf
              <div class="mb-3">
                <label class="form-label">{{ __('app.role_name') }}</label>
                <input type="text" name="name" class="form-control" placeholder="{{ __('app.role_name_placeholder') }}" required>
              </div>
      ...
      <div class="col-12 mb-4">
        <div class="card h-100">
          <div class="card-body">
            ...
            <form method="POST" action="{{ route('tenant.subdomain.roles.attach', ['subdomain' => request()->route('subdomain')]) }}">
              @csrf
              <div class="mb-3">
