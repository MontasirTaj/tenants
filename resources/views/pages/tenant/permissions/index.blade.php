        ...
        <div class="col-xl-8 mx-auto">
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <form method="POST"
                        action="{{ route('tenant.subdomain.permissions.store', ['subdomain' => request()->route('subdomain')]) }}"
                        class="mb-0">
                        @csrf
                        <div class="row align-items-end">
                            <div class="col-md-8 mb-3 mb-md-0">
                                ...
                                <input type="text" name="name" class="form-control"
                                    placeholder="{{ __('app.permission_name_placeholder') }}" required>
                            </div>
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3">
                        ...
                        <div class="mt-3 mt-md-0">
                            <div class="btn-group btn-group-sm" role="group" aria-label="{{ __('app.export') }}">
                                <a href="{{ route('tenant.subdomain.permissions.export.excel', ['subdomain' => $sub]) }}"
                                    class="btn btn-outline-success">
                                    <i class="mdi mdi-file-excel-outline"></i>
                                    <span>{{ __('app.export_excel') }}</span>
                                </a>
                                <a href="{{ route('tenant.subdomain.permissions.export.pdf', ['subdomain' => $sub]) }}"
                                ...
                                @foreach ($permissions as $p)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('tenant.subdomain.permissions.edit', ['subdomain' => $sub, 'permission' => $p->id]) }}"
                                                class="btn btn-sm btn-outline-primary tenant-action-btn">
                                                <i class="mdi mdi-pencil-outline"></i>
                                                <span>{{ __('app.edit') }}</span>
                                            </a>
                                            <form method="POST"
                                                action="{{ route('tenant.subdomain.permissions.destroy', ['subdomain' => $sub, 'permission' => $p->id]) }}"
                                                onsubmit="return confirm('{{ __('app.confirm_delete_permission') }}');"
